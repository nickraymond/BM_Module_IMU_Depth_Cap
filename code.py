...
SENSOR_POLL_INTERVAL_MS = 1000  # 250 ms (4 Hz)
...
while True:
    ...
    if current_time >= next_poll_time:
        ...
        if sensor.read():
            ...
            poll_log_message = f"{int(current_time)}t,{capacitive_value},{pressure:.0f},{temperature / 10:.1f},{tilt:.2f},{roll / 10:.1f},{pitch / 10:.1f},{yaw / 10:.1f},{accel_x:.2f},{accel_y:.2f},{accel_z:.2f}"
            #poll_log_message = f"Cap.: {capacitive_value} | Pres: {pressure:.0f} μbar | Temp: {temperature / 10:.1f} °C | Tilt: {tilt:.1f} | Roll: {roll / 10:.1f} | Pitch: {pitch / 10:.1f} | Yaw: {yaw / 10:.1f} | Accel X: {accel_x:.1f} | Accel Y: {accel_y:.1f} | Accel Z: {accel_z:.1f}"

            # Save sensor poll data to SD card
            bm.spotter_log("tilt_data.log", poll_log_message)
            ...
            print(f"DATA | Cap.: {capacitive_value} | Pres: {pressure:.0f} μbar | Temp: {temperature / 10:.1f} °C | Tilt: {tilt:.1f} | Roll: {roll / 10:.1f} | Pitch: {pitch / 10:.1f} | Yaw: {yaw / 10:.1f} | Accel X: {accel_x:.1f} | Accel Y: {accel_y:.1f} | Accel Z: {accel_z:.1f} | Delta: {delta_time:.0f} ms")
